...
This project provides the batteries and utilities needed to run your pipelines, but does not offer hosting the logic of any pipelines themselves. This approach offers the benefit of making all components more modular, manageable and decoupled. The pipelines logic would be hosted in a separate repository.
...
```python
 ...
 def run_cpg_flow(dry_run=False):
    ...
    # See the 'Key Considerations and Limitations' section for this definition below:
...
### `stages.py` or equivalent file for the `Stage` definitions

A `Stage` represents a node in the DAG. The stages can be abstracted from either a `CohortStage`, `MultiCohortStage`, or a `SequencingGroupStage`.

The stage definition should use the `@stage` decorator to set what stages to depend on, set analysis keys and the analysis type. All stages require an `expected_outputs` class method definition, that sets the expected output path location for a given `Target` such as a `SequencingGroup`, `Cohort`, or `MultiCohort`.
